...
namespace Genbox.Wikipedia
{
    public class WikipediaClient
    {
        ...
        private RestRequest CreatePageRequest(string title) {
            //API example: https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&titles=Stack%20Overflow
            _client.BaseUrl = new Uri(string.Format(UseTls ? "https://{0}.wikipedia.org/w/" : "http://{0}.wikipedia.org/w/", Language.GetStringValue()));
            ...
            request.AddParameter("explaintext", 1);
        ...
        public PageResult GetPage(string title) {
            RestRequest request = CreatePageRequest(title);
            IRestResponse response = _client.Execute(request);
        ...
        public async Task<PageResult> GetPageAsync(string title, CancellationToken token = default) {
            RestRequest request = CreatePageRequest(title);
            IRestResponse response = await _client.ExecuteAsync(request, token).ConfigureAwait(false);
            return DeserializePage(response);
        ...
        private PageResult DeserializePage(IRestResponse response) {
            IDeserializer deserializer;
            ...
            PageResult results = deserializer.Deserialize<PageResult>(response);
